...
class OnConstraintPolicyRunner:
    ...
    def __init__(self,
        ...
        actor_critic: ActorCriticRMA = actor_critic_class(self.env.cfg.env.n_proprio,
                                                      ...
                                                      **self.policy_cfg)
        if self.cfg['resume']:
            resume_path = os.path.join(ROOT_DIR, self.cfg['resume_path'])
            print(f"Loading actor/critic checkpoint from: {resume_path}")
    ...
    def _freeze_actor_inference_normalization(self):
        """Preserve normalization statistics from a resumed walking policy."""
        backbone = getattr(self.alg.actor_critic, "actor_teacher_backbone", None)
        if backbone is None:
            return

        obs_normalizer = getattr(backbone, "obs_normalizer", None)
        if obs_normalizer is not None:
            obs_normalizer.eval()

        # These two BatchNorm blocks are on the action-inference path.  The
        # projector BatchNorm used by the Barlow loss intentionally stays in
        # training mode.
        for block_name in ("mlp_encoder", "latent_layer"):
            block = getattr(backbone, block_name, None)
            if block is None:
                continue
    ...
    def learn(self, num_learning_iterations, init_at_random_ep_len=False):
        ...
        self._freeze_actor_inference_normalization()
    ...
    def load(self, path, load_optimizer=True):
        ...
        # Older Panda checkpoints store the complete actor/critic and optimizer
        # but predate the separately saved estimator entry. Keep resume
        # backward-compatible instead of failing with KeyError.
        if 'estimator_state_dict' in loaded_dict:
            self.alg.estimator.load_state_dict(loaded_dict['estimator_state_dict'])
        else:
            warnings.warn(
                "Checkpoint has no estimator_state_dict; using the newly "
                "initialized estimator while strictly loading actor_critic."
            )
        if self.if_depth:
