...
class BaseTask():
    ...
    def __init__(self, cfg, sim_params, physics_engine, sim_device, headless, eval_cfg=None):
        ...
        if self.headless == False:
            ...
            self.gym.subscribe_viewer_keyboard_event(
                self.viewer, gymapi.KEY_Q, "vy_plus")
            self.gym.subscribe_viewer_keyboard_event(
                self.viewer, gymapi.KEY_E, "vy_minus")
            self.gym.subscribe_viewer_keyboard_event(
                self.viewer, gymapi.KEY_X, "command_stop")
            # Recovery evaluation controls consumed by simple_play.py.
            self.gym.subscribe_viewer_keyboard_event(
                self.viewer, gymapi.KEY_U, "recovery_upside")
            self.gym.subscribe_viewer_keyboard_event(
                self.viewer, gymapi.KEY_J, "recovery_left")
            self.gym.subscribe_viewer_keyboard_event(
                self.viewer, gymapi.KEY_K, "recovery_right")
            self.gym.subscribe_viewer_keyboard_event(
                self.viewer, gymapi.KEY_R, "recovery_random")
            self.gym.subscribe_viewer_keyboard_event(
                self.viewer, gymapi.KEY_T, "recovery_stand")
        self.free_cam = False
        self.lookat_id = 0
        self.keyboard_recovery_request = None
    ...
    def render(self, sync_frame_time=True):
        if self.viewer:
            ...
            for evt in self.gym.query_viewer_action_events(self.viewer):
                ...
                # Number keys always select the recovery-test target.  In free
                # camera mode they do not move the camera, so the 3x3 overview
                # remains visible.
                ...
                recovery_actions = {
                    "recovery_upside": "upside",
                    "recovery_left": "left",
                    "recovery_right": "right",
                    "recovery_random": "random",
                    "recovery_stand": "stand",
                }
                if evt.value > 0 and evt.action in recovery_actions:
                    self.keyboard_recovery_request = recovery_actions[evt.action]
                ...
                if not self.free_cam:
                    ...
                    if evt.action == "right_turn" and evt.value > 0:
                        self.commands[self.lookat_id, 3] -= 0.5
                    if evt.action == "command_stop" and evt.value > 0:
                        self.commands[self.lookat_id, :] = 0.
