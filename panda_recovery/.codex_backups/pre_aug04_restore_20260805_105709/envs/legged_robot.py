...
class LeggedRobot(BaseTask):
    ...
    def _init_buffers(self):
        ...
        self.recovery_active = torch.zeros(self.num_envs, dtype=torch.bool, device=self.device, requires_grad=False)
        self.recovery_stable_steps = torch.zeros(self.num_envs, dtype=torch.long, device=self.device, requires_grad=False)
        self.recovery_just_succeeded = torch.zeros(self.num_envs, dtype=torch.bool, device=self.device, requires_grad=False)
        self.recovery_timeout_buf = torch.zeros(self.num_envs, dtype=torch.bool, device=self.device, requires_grad=False)
        self.last_recovery_upright = torch.zeros(self.num_envs, dtype=torch.float, device=self.device, requires_grad=False)
    ...
    def _post_physics_step_callback(self):
        ...
        """
        self.recovery_just_succeeded[:] = False
    ...
    def check_termination(self):
        ...
        self.reset_buf = torch.any(torch.norm(self.contact_forces[:, self.termination_contact_indices, :], dim=-1) > 1.,
                                   dim=1)
        self.reset_buf |= self.recovery_timeout_buf
    ...
    def reset_idx(self, env_ids):
        ...
        self.recovery_timeout_buf[env_ids] = False
        self.recovery_just_succeeded[env_ids] = False
        self._resample_commands(env_ids)

        if self.cfg.domain_rand.randomize_init_state and self.cfg.domain_rand.recovery_push_robots:
            self._activate_recovery(env_ids)
    ...
    def _reset_root_states(self, env_ids):
        ...
        if self.cfg.domain_rand.randomize_init_state:
            ...
            # Make a fixed fraction of episode resets explicit, nearly static
            # feet-up samples. This ensures the policy sees the deployment
            # failure case without first having to survive until a mid-episode
            # recovery event.
            upside_down_mask = torch.rand(len(env_ids), device=self.device) \
                < self.cfg.domain_rand.recovery_init_upside_down_ratio
            upside_down_ids = env_ids[upside_down_mask]
            if len(upside_down_ids) > 0:
                ...
                velocity_noise = self.cfg.domain_rand.recovery_upside_down_velocity_noise
                ...
                flip_angle = flip_sign * (
                    np.pi - torch_rand_float(
                        0., angle_noise, (count, 1), device=self.device
                    ).squeeze(1)
                )
                small_roll = torch_rand_float(
                    -tilt_noise, tilt_noise, (count, 1), device=self.device
                ).squeeze(1)
                ...
                yaw = torch_rand_float(
                    -np.pi, np.pi, (count, 1), device=self.device
                ).squeeze(1)
                ...
                self.root_states[upside_down_ids, 7:13] = torch_rand_float(
                    -velocity_noise, velocity_noise, (count, 6), device=self.device
                )
    ...
    def _parse_cfg(self, cfg):
        ...
        self.cfg.domain_rand.recovery_max_steps = np.ceil(self.cfg.domain_rand.recovery_max_time_s / self.dt)
        self.cfg.domain_rand.recovery_stable_steps = np.ceil(self.cfg.domain_rand.recovery_stable_time_s / self.dt)
    ...
    def _get_recovery_curriculum_values(self):
        ...
        if self.cfg.domain_rand.recovery_curriculum:
            progress = min(float(self.global_counter) / float(self.cfg.domain_rand.recovery_curriculum_steps), 1.)
    ...
    def _recovery_push_robots(self):
        ...
        if randomize_ratio > 0.:
            ...
            if torch.any(randomize_envs):
                ...
                if len(upside_down_ids) > 0:
                    ...
                    tilt_noise = self.cfg.domain_rand.recovery_upside_down_tilt_noise
                    velocity_noise = self.cfg.domain_rand.recovery_upside_down_velocity_noise
                    min_flip_angle = np.pi - self.cfg.domain_rand.recovery_upside_down_angle_noise
                    if self.cfg.domain_rand.recovery_upside_down_curriculum:
                        upside_progress = min(
                            float(self.global_counter) /
                            float(self.cfg.domain_rand.recovery_upside_down_curriculum_steps),
                            1.,
                        )
                        initial_angle = self.cfg.domain_rand.recovery_upside_down_initial_min_angle
                        final_angle = self.cfg.domain_rand.recovery_upside_down_final_min_angle
                        min_flip_angle = initial_angle + upside_progress * (final_angle - initial_angle)
                    flip_roll = torch.rand(count, device=self.device) < 0.5
                    flip_sign = torch.where(
                        torch.rand(count, device=self.device) < 0.5,
                        -torch.ones(count, device=self.device),
                    ...
                    flip_angle = flip_sign * torch_rand_float(
                        min_flip_angle, np.pi, (count, 1), device=self.device
                    ).squeeze(1)
                    ...
                    self.root_states[upside_down_ids, 3:7] = quat_from_euler_xyz_local(roll, pitch, yaw)
                    # Train active self-righting from rest. Large injected
                    # angular velocity lets the policy rely on passive rolling
                    # and does not match a robot lying still on its back.
                    self.root_states[upside_down_ids, 7:13] = torch_rand_float(
                        -velocity_noise, velocity_noise, (count, 6), device=self.device
                    )
        ...
        self._activate_recovery(recovery_ids)
        self.gym.set_actor_root_state_tensor(self.sim, gymtorch.unwrap_tensor(self.root_states))

    def _activate_recovery(self, env_ids):
        """Start a recovery attempt and hide locomotion commands until success."""
        if len(env_ids) == 0:
            return
        self.recovery_commands[env_ids] = self.commands[env_ids]
        self.recovery_command_timer[env_ids] = int(self.cfg.domain_rand.recovery_max_steps)
        self.recovery_stable_steps[env_ids] = 0
        self.recovery_active[env_ids] = True
        self.recovery_timeout_buf[env_ids] = False
        self.commands[env_ids] = 0.

        local_gravity = quat_rotate_inverse(self.root_states[env_ids, 3:7], self.gravity_vec[env_ids])
        self.last_recovery_upright[env_ids] = torch.clamp((1. - local_gravity[:, 2]) * 0.5, 0., 1.)

    def _process_recovery_command_hold(self):
        """Keep commands zero until stable standing, or reset a timed-out attempt."""
        active = self.recovery_active
        if not torch.any(active):
            return

        self.commands[active, :] = 0.
        stand_ready = active & self._get_recovery_stand_mask()
        self.recovery_stable_steps[active & ~stand_ready] = 0
        self.recovery_stable_steps[stand_ready] += 1

        self.recovery_command_timer[active] -= 1
        succeeded = active & (
            self.recovery_stable_steps >= int(self.cfg.domain_rand.recovery_stable_steps)
        )
        timed_out = active & (self.recovery_command_timer <= 0) & ~succeeded

        if torch.any(succeeded):
            self.recovery_just_succeeded[succeeded] = True
            self.recovery_active[succeeded] = False
            self.commands[succeeded] = self.recovery_commands[succeeded]
        if torch.any(timed_out):
            self.recovery_active[timed_out] = False
            self.recovery_timeout_buf[timed_out] = True

    def _get_recovery_stand_mask(self):
        """Return environments satisfying the shared recovery-success test."""
        base_height = self._get_base_heights()
        upright = torch.clamp(-self.projected_gravity[:, 2], 0., 1.)
        contact_count = torch.sum(1. * self.contact_filt, dim=-1)
        return (base_height > self.cfg.domain_rand.recovery_success_height_ratio
                * self.cfg.rewards.base_height_target) \
            & (upright > self.cfg.domain_rand.recovery_success_upright) \
            & (contact_count >= self.cfg.domain_rand.recovery_success_min_contacts)
    ...
    def _resample_commands(self, env_ids):
        ...
        active = self.recovery_active[env_ids]
        if torch.any(active):
    ...
    def _reward_base_height_low(self):
        base_height = self._get_base_heights()
        height_cost = torch.square(torch.clamp(self.cfg.rewards.base_height_target - base_height, min=0.))
        # Do not let a large height penalty erase all exploration signal while
        # the robot is still inverted. Restore the full penalty as it becomes
        # upright; normal locomotion is unchanged.
        upright_score = torch.clamp((1. - self.projected_gravity[:, 2]) * 0.5, 0., 1.)
        recovery_gate = torch.where(self.recovery_active, upright_score, torch.ones_like(upright_score))
        return height_cost * recovery_gate
    ...
    def _reward_base_height_progress(self):
        ...
        return height_progress * (0.25 + 0.75 * upright_progress)
    ...
    def _reward_foot_slide_up(self):
        ...
        cost_slide = torch.sum(self.contact_filt * foot_lateral_vel, dim=1) * torch.clamp(-self.projected_gravity[:,2],0,1)
        return cost_slide
    ...
    def _reward_stand_success(self):
        recovery_phase = self.recovery_active | self.recovery_just_succeeded
        return 1. * self._get_recovery_stand_mask() * recovery_phase
    ...
    def _reward_stand_ready(self):
        ...
        recovery_phase = self.recovery_active | self.recovery_just_succeeded
        return height_progress * upright_progress * contact_fraction * recovery_phase

    def _reward_recovery_upright(self):
        upright_score = torch.clamp((1. - self.projected_gravity[:, 2]) * 0.5, 0., 1.)
        return upright_score * self.recovery_active

    def _reward_recovery_progress(self):
        """Reward step-to-step rotation toward upright during recovery."""
        upright_score = torch.clamp((1. - self.projected_gravity[:, 2]) * 0.5, 0., 1.)
        recovery_phase = self.recovery_active | self.recovery_just_succeeded
        progress = torch.clamp(upright_score - self.last_recovery_upright, min=0.) * recovery_phase
        self.last_recovery_upright[recovery_phase] = upright_score[recovery_phase]
        return progress

    def _reward_upside_escape(self):
        """Give a dense bridge from exactly feet-up to a side-lying pose."""
        gravity_z = self.projected_gravity[:, 2]
        escape = torch.clamp(1. - torch.abs(gravity_z), 0., 1.)
        return escape * (gravity_z > 0.) * self.recovery_active

    def _reward_recovery_motion(self):
        """Small exploration bonus for initiating a roll while deeply inverted."""
        deeply_inverted = self.projected_gravity[:, 2] > 0.5
        roll_rate = torch.clamp(torch.norm(self.base_ang_vel[:, :2], dim=1), max=2.)
        return roll_rate * deeply_inverted * self.recovery_active

    def _reward_feet_below_base(self):
        feet_below_base = self.root_states[:, 2].unsqueeze(1) - self.feet_pos[:, :, 2]
        return torch.mean(torch.clamp(feet_below_base / self.cfg.rewards.base_height_target, 0., 1.), dim=1)
    ...
    def _reward_dof_acc(self):
        ...
        penalty = torch.sum(torch.square((self.last_dof_vel - self.dof_vel) / self.dt), dim=1)
        return penalty * (~self.recovery_active).float()
    ...
    def _reward_action_smoothness(self):
        penalty = torch.sum(torch.square(
            self.action_history_buf[:, -1, :] - 2 * self.action_history_buf[:, -2, :]
    ...
    def _reward_tracking_lin_vel(self):
        ...
        return torch.exp(-lin_vel_error/self.cfg.rewards.tracking_sigma) * ~self.recovery_active
    
    def _reward_tracking_ang_vel(self):
        # Tracking of angular velocity commands (yaw) 
        ang_vel_error = torch.square(self.commands[:, 2] - self.base_ang_vel[:, 2])
        return torch.exp(-ang_vel_error/self.cfg.rewards.tracking_sigma) * ~self.recovery_active
